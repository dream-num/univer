...
电子表格 · 文档 · 演示文稿 · 画布 · 关系表格 · PDF（即将推出）

用于构建可嵌入生产力体验的全栈、同构 Office SDK。

插件化架构、Canvas 渲染引擎、公式引擎和统一的 Facade API，
同时支持浏览器和 Node.js。
...
以下开源项目基于 Univer Office SDK 构建，将办公工具带入工作空间、命令行工作流和 Agent 环境：
