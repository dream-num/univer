...
export interface IDeleteRangeParams {
    workbookId: string;
    worksheetId: string;
    range: IRangeData;
 ...
 */
export const DeleteRangeCommand: ICommand = {
    type: CommandType.COMMAND,
    id: 'sheet.command.delete-range',

    handler: async (accessor: IAccessor, params: IDeleteRangeParams) => {
        const commandService = accessor.get(ICommandService);
        const undoRedoService = accessor.get(IUndoRedoService);
        const { range, workbookId, worksheetId, shiftDimension } = params;
