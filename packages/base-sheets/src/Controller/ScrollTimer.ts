import { Viewport, Scene, Vector2, requestNewFrame, cancelRequestFrame, RENDER_CLASS_TYPE } from '@univer/base-render';
...
export class ScrollTimer {
    ...
    startScroll(offsetX: number, offsetY: number) {
        ...
        this._viewport = this._getViewportByCoord(this._scene);
    ...
    _scroll(viewport: Nullable<Viewport>) {
        ...
        if (limited) {
            ...
            const newViewport = this._getViewportByCoord(ancestorScene);
            if (newViewport) {
    ...
    scrolling(offsetX: number, offsetY: number, scrollFunction: () => void) {
        this._moveX = offsetX;
        this._moveY = offsetY;
    ...
    private _runRenderLoop() {
        ...
        this._scrollFunction && this._scrollFunction();
    ...
    private _getViewportByCoord(scene?: Scene) {
