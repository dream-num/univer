import { BaseComponentProps, BaseComponentRender, BaseComponentSheet, Component, createRef, RefObject } from '@univer/base-component';
import { PLUGIN_NAMES } from '@univer/core';
import { IToolBarItemProps } from '../../../Model/ToolBarModal1';
...
export class ToolBar1 extends Component<IProps, IState> {
    ...
    resetLabel = (toolList: any[]) => {
        ...
        for (let i = 0; i < toolList.length; i++) {
            ...
            // 优先寻找自定义组件
            const component = plugin?.getRegisterComponent(item.label);
            if (component) {
            ...
            const suffixComponent = plugin?.getRegisterComponent(item.suffix);
            if (suffixComponent) {
                const Suffix = suffixComponent.component;
                const props = suffixComponent.props ?? {};
    ...
    componentDidMount() {
        this._context.getObserverManager().getObserver<ToolBar1>('onToolBar1DidMountObservable')?.notifyObservers(this);
    }
