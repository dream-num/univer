...
export class CopyExtensionManager {
    ...
    private _handleTableData(table: string, property: IKeyValue) {
        ...
        const insertIndex = table.indexOf('<table') + '<table'.length; // 插入位置
        const result = table.slice(0, insertIndex) + propertyStr + table.slice(insertIndex); // 插入新字符串
    ...
    private _insertEmbedFlag(table: string, embed: boolean): string {
        const insertIndex = table.indexOf('<table') + '<table'.length; // 插入位置
        const result = `${table.slice(0, insertIndex)}embed="${embed}"${table.slice(insertIndex)}`; // 插入新字符串
