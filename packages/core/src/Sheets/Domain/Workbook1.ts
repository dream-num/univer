 ...
 */
export class Workbook1 {
    /**
     * sheets list
     * @private
     */
    private _worksheets: Map<string, Worksheet1>;
    ...
    private _getDefaultWorkSheet(): void {
        ...
        for (let sheetId in sheets) {
            ...
            const worksheet = new Worksheet1(_context, config);
    ...
    private _initialize(): void {
        ...
        this._worksheets = new Map<string, Worksheet1>();
    ...
    _getWorksheets(): Map<string, Worksheet1> {
        return this._worksheets;
    }

    nextSheet(start: number): Nullable<Worksheet1> {
        if (start >= 0) {
    ...
    insertSheet(sheet: Worksheet1): Nullable<string>;
    ...
    insertSheet(index: number, sheet: Worksheet1): Nullable<string>;
    insertSheet(...argument: any[]): Nullable<string> {
        ...
        if (Tools.hasLength(argument, 1)) {
            ...
            if (Tools.isAssignableFrom(argument[0], Worksheet1)) {
        ...
        if (Tools.hasLength(argument, 2)) {
            ...
            if (Tools.isNumber(argument[0])) {
                ...
                if (Tools.isAssignableFrom(argument[1], Worksheet1)) {
                    const index = argument[0];
    ...
    getActiveSpreadsheet(): Workbook1 {
        return this;
    }
    ...
    create(name: string, row: number, column: number): Worksheet1;
    create(name: string): Worksheet1;
    create(...argument: unknown[]): unknown {
        if (Tools.hasLength(argument, 1)) {
            const { _container, _context } = this;
            const name = argument[0];
            const conf = { ...DEFAULT_WORKSHEET, name };
            const worksheet = _container.getInstance<Worksheet1>(
                'WorkSheet',
                _context,
                conf
            );
    ...
    getActiveSheet(): Worksheet1 {
        const { sheetOrder } = this._config;
        const activeSheetId = sheetOrder.find((sheetId) => {
            const worksheet = this._worksheets.get(sheetId) as Worksheet1;
        ...
        return this._worksheets.get(activeSheetId) as Worksheet1;
    }
    ...
    getSheetIndex(sheet: Worksheet1): number {
        const { sheetOrder } = this._config;
        return sheetOrder.findIndex((sheetId) => {
    ...
    getSheetBySheetName(name: string): Nullable<Worksheet1> {
        const { sheetOrder } = this._config;
    ...
    getSheetBySheetId(sheetId: string): Nullable<Worksheet1> {
        return this._worksheets.get(sheetId);
    }
     ...
     */
    setActiveSheet(sheet: Worksheet1): Worksheet1;
    /**
     ...
     */
    setActiveSheet(sheet: Worksheet1, restoreSelection: boolean): Worksheet1;
    setActiveSheet(...argument: any): Worksheet1 {
        let restoreSelection = false;
        const worksheet: Worksheet1 = argument[0];
        if (Tuples.checkup(argument, Worksheet1, Tuples.BOOLEAN_TYPE)) {
