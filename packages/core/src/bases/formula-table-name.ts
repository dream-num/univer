...
type BaseFormulaTable = Pick<ITableSnapshot, 'id' | 'name'>;
...
export function createBaseFormulaTableNameMap(snapshot: BaseFormulaSnapshot): ReadonlyMap<string, string> {
    ...
    for (const table of tables) {
        const baseName = normalizeBaseFormulaTableName(table.name);
...
export function migrateBaseFormulaTableNames(snapshot: IBaseSnapshot): void {
    const aliases = createBaseFormulaTableAliases(snapshot);
...
function createBaseFormulaTableAliases(snapshot: BaseFormulaSnapshot): Array<{ alias: string; formulaName: string }> {
    ...
    return Object.values(snapshot.tables)
        .flatMap((table) => {
            ...
            const inputAliases = [table.id, createLegacyBaseFormulaTableName(table.id)];
