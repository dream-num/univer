...
export class PluginService implements IDisposable {
    ...
    private _assertPluginValid(ctor: PluginCtor<Plugin>): void {
        ...
        if (this._seenPlugins.has(pluginName)) {
            throw new Error(`[PluginService]: duplicate plugin name for "${pluginName}". Maybe a plugin that dependents on "${pluginName} has already registered it. In that case please register "${pluginName}" before the that plugin.`);
        }
