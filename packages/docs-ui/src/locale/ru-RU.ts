/**
 * Copyright 2023-present DreamNum Co., Ltd.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

import type enUS from './en-US';

const locale: typeof enUS = {
    'docs-ui': {
        pasteOptions: {
            title: 'Параметры вставки',
            source: 'Сохранить исходное форматирование',
            destination: 'Использовать форматирование назначения',
            text: 'Сохранить только текст',
            useKeyboard: 'Нет доступа к буферу обмена. Используйте Ctrl+V (Mac: ⌘V) или Ctrl+Shift+V (Mac: ⌘⇧V) для вставки только текста.',
            useSystemPaste: 'Нет доступа к буферу обмена. Используйте функцию вставки устройства или клавиатуры, затем измените формат через параметры вставки.',
            failed: 'Не удалось изменить формат вставки.',
        },
        outline: {
            title: 'Структура документа',
        },
        objectPermission: {
            paragraphPermission: 'Права доступа',
            title: 'Права доступа',
            paragraph: 'Абзац',
            section: 'Раздел',
        },
        mobile: {
            insert: 'Вставка',
            edit: 'Редактировать',
            done: 'Готово',
            more: 'Ещё',
            back: 'Назад',
            close: 'Закрыть',
            format: 'Формат',
            layout: 'Макет',
            keyboard: 'Показать клавиатуру',
            hideKeyboard: 'Скрыть клавиатуру',
            expand: 'Развернуть инструменты',
            collapse: 'Свернуть инструменты',
        },
        layout: {
            progress: 'Выполняется разметка документа…',
            recovering: 'Восстановление макета документа…',
        },
        toolbar: {
            font: 'Шрифт',
            fontSize: 'Размер шрифта',
            bold: 'Полужирный',
            italic: 'Курсив',
            strikethrough: 'Зачеркнутый',
            subscript: 'Нижний индекс',
            superscript: 'Верхний индекс',
            underline: 'Подчеркнутый',
            textColor: {
                main: 'Цвет текста',
            },
            fillColor: {
                main: 'Цвет фона текста',
            },
            table: {
                main: 'Таблица',
                insert: 'Добавить таблицу',
                colCount: 'Количество столбцов',
                rowCount: 'Количество строк',
            },
            resetColor: 'Сбросить',
            order: 'Упорядоченный список',
            unorder: 'Неупорядоченный список',
            checklist: 'Список задач',
            alignLeft: 'Выровнять по левому краю',
            alignCenter: 'Выровнять по центру',
            alignRight: 'Выровнять по правому краю',
            alignJustify: 'Выровнять по ширине',
            horizontalLine: 'Горизонтальная линия',
            headerFooter: 'Верхние и нижние колонтитулы',
            pageSetup: 'Настройки страницы',
            breaks: 'Разрывы',
            columnBreak: 'Разрыв колонки',
            sectionBreakNextPage: 'Разрыв раздела (Со следующей страницы)',
            sectionBreakContinuous: 'Разрыв раздела (Непрерывный)',
            sectionBreakNextColumn: 'Разрыв раздела (Со следующей колонки)',
            sectionBreakEvenPage: 'Разрыв раздела (С чётной страницы)',
            sectionBreakOddPage: 'Разрыв раздела (С нечётной страницы)',
            heading: {
                tooltip: 'Заголовок',
                normal: 'Обычный текст',
                leading1: 'Заголовок 1',
                leading2: 'Заголовок 2',
                leading3: 'Заголовок 3',
                leading4: 'Заголовок 4',
                leading5: 'Заголовок 5',
                title: 'Название',
                subTitle: 'Подзаголовок',
            },
        },
        table: {
            insert: 'Вставить',
            insertRowAbove: 'Вставить строку выше',
            insertRowBelow: 'Вставить строку ниже',
            insertColumnLeft: 'Вставить столбец слева',
            insertColumnRight: 'Вставить столбец справа',
            delete: 'Удалить таблицу',
            deleteRows: 'Удалить строку',
            deleteColumns: 'Удалить столбец',
            deleteTable: 'Удалить таблицу',
        },
        headerFooter: {
            linkToPrevious: 'Связать с предыдущим',
            header: 'Верхний колонтитул',
            footer: 'Нижний колонтитул',
            panel: 'Настройки верхнего и нижнего колонтитулов',
            firstPageCheckBox: 'Особенная первая страница',
            oddEvenCheckBox: 'Разные четные и нечетные страницы',
            headerTopMargin: 'Верхнее поле верхнего колонтитула (px)',
            footerBottomMargin: 'Нижнее поле нижнего колонтитула (px)',
            closeHeaderFooter: 'Закрыть верхний и нижний колонтитулы',
            disableText: 'Настройки верхнего и нижнего колонтитулов отключены',
        },
        statistics: {
            title: 'Статистика документа',
            open: 'Показать статистику документа',
            document: 'Весь документ',
            selection: 'Текущее выделение',
            wordCount: 'Слов: {0}',
            selectedWords: 'Слов: {0} из {1}',
            currentPage: 'Страница {0}',
            currentPageOfTotal: 'Страница {0} из {1}',
            pages: 'Страницы',
            words: 'Слова',
            charactersWithoutSpaces: 'Знаки (без пробелов)',
            charactersWithSpaces: 'Знаки (с пробелами)',
            paragraphs: 'Абзацы',
            lines: 'Строки',
            nonAsianWords: 'Неазиатские слова',
            asianCharactersAndKoreanWords: 'Азиатские знаки и корейские слова',
        },
        placeholder: {
            heading1: 'Заголовок 1',
            heading2: 'Заголовок 2',
            heading3: 'Заголовок 3',
            heading4: 'Заголовок 4',
            heading5: 'Заголовок 5',
            normalText: 'Введите текст или нажмите «/» для вызова команд',
            listItem: 'Элемент',
        },
        doc: {
            blockMenu: {
                dragBlock: 'Перетащить блок',
            },

            menu: {
                paragraphSetting: 'Настройка абзаца',
                sectionSetting: 'Параметры раздела',
            },
            slider: {
                paragraphSetting: 'Настройка абзаца',
                sectionSetting: 'Параметры раздела',
            },
            paragraphSetting: {
                alignment: 'Выравнивание',
                indentation: 'Отступ',
                left: 'Левый',
                right: 'Правый',
                firstLine: 'Первая строка',
                hanging: 'Висит',
                spacing: 'Расстояние',
                before: 'До',
                after: 'После',
                lineSpace: 'Высота строки',
                multiSpace: 'Двойной отступ',
                atLeast: 'Не менее (px)',
                exactly: 'Точно (px)',
            },
            sectionSetting: {
                selectedSections: 'Выбрано разделов: {0}',
                multipleValues: 'Несколько значений',
                columnCount: 'Количество колонок',
                columnGap: 'Интервал между колонками',
                columnSeparator: 'Разделитель',
                none: 'Нет',
                betweenColumns: 'Между колонками',
                sectionStart: 'Начало раздела',
                unspecified: 'Не указано',
                continuous: 'Непрерывный',
                nextColumn: 'Следующая колонка',
                nextPage: 'Следующая страница',
                evenPage: 'Чётная страница',
                oddPage: 'Нечётная страница',
                pageSetup: 'Параметры страницы',
                pageWidth: 'Ширина страницы',
                pageHeight: 'Высота страницы',
                pageNumberStart: 'Начальный номер страницы',
            },
        },
        rightClick: {
            copy: 'Копировать',
            cut: 'Вырезать',
            paste: 'Вставить',
            select: 'Выбрать',
            selectAll: 'Выбрать всё',
            delete: 'Удалить',
            bulletList: 'Неупорядоченный список',
            orderList: 'Упорядоченный список',
            checkList: 'Список задач',
            insertBellow: 'Вставить ниже',
        },
        paragraphMenu: {
            alignAndIndent: 'Выравнивание и отступ',
            align: 'Выравнивание',
            indent: 'Отступ',
            color: 'Цвета',
            increase: 'Увеличить',
            decrease: 'Уменьшить',
            increaseIndent: 'Увеличить отступ',
            decreaseIndent: 'Уменьшить отступ',
            defaultTextColor: 'Цвет текста по умолчанию',
            noBackground: 'Без фона',
        },
        'page-settings': {
            'document-setting': 'Настройки документа',
            'modern-width': 'Ширина содержимого',
            'modern-width-narrow': 'Узкая',
            'modern-width-medium': 'Средняя',
            'modern-width-wide': 'Широкая',
            'page-size': {
                main: 'Размер бумаги',
                a4: 'A4',
                a3: 'A3',
                a5: 'A5',
                b4: 'B4',
                b5: 'B5',
                letter: 'Американский формат',
                legal: 'Юридический формат',
                tabloid: 'Таблоидный формат',
                statement: 'Формат заявления',
                executive: 'Исполнительный формат',
                folio: 'Формат фолио',
            },
            'paper-size': 'Размер бумаги',
            orientation: 'Ориентация',
            portrait: 'Книжная',
            landscape: 'Альбомная',
            'custom-paper-size': 'Пользовательский размер бумаги',
            top: 'Верх',
            bottom: 'Низ',
            left: 'Слева',
            right: 'Справа',
            cancel: 'Отмена',
            confirm: 'Подтвердить',
        },
    },
};

export default locale;
