...
describe('core editing commands', () => {
    ...
    it('allows a valid paragraph replacement that does not add to legacy structure defects', () => {
        testBed.univer.dispose();
        testBed = createTestBed({
            id: 'test',
            body: {
                dataStream: 'A\b\r\n',
                paragraphs: [{ startIndex: 2, paragraphId: 'body' }],
                sectionBreaks: [{ sectionId: 'section_fixture_legacy', startIndex: 3 }],
            },
            documentStyle: {},
        });
        commandService = testBed.get(ICommandService);
        ...
        expect(testBed.doc.getBody()?.dataStream).toBe('A\r\b\r\n');
    });
