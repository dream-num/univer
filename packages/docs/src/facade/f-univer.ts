...
export interface IFUniverDocsMixin {
     ...
     * @param {ICreateUnitOptions} options Unit creation options, including embedded rendering without changing the active host.
...
export class FUniverDocsMixin extends FUniver implements IFUniverDocsMixin {
    override createDocument(data: Partial<IDocumentData>, options?: ICreateUnitOptions): FDocument {
        ...
        const document = instanceService.createUnit<IDocumentData, DocumentDataModel>(UniverInstanceType.UNIVER_DOC, data, options);
