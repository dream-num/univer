...
describe('embed runtime slots', () => {
    ...
    it('adds runtime slot CSS once', () => {
        const firstHost = document.createElement('div');
        const secondHost = document.createElement('div');

        const firstDisposable = ensureEmbedDefaultRuntimeSlots(firstHost);
        const secondDisposable = ensureEmbedDefaultRuntimeSlots(secondHost);
        const styles = [...document.querySelectorAll('style#univer-embed-runtime-slot-styles')];

        expect(styles).toHaveLength(1);
        expect(styles[0].textContent).toContain('.univer-embed-runtime-slot-content');
        expect(styles[0].textContent).toContain('overflow: hidden');
        expect(styles[0].textContent).toContain('.univer-embed-runtime-slot-canvas');
        expect(styles[0].textContent).toContain('pointer-events: none');
        expect(styles[0].textContent).toContain('.univer-embed-runtime-slot-canvas > *');
