...
describe('Test FWorkbookEngineFormulaMixin', () => {
    ...
    it('should get formula errors in a specific range', () => {
        ...
        // Test range that contains some errors
        const range = {
        ...
        const errors = workbook.getFormulaErrorByRange(range);
        ...
        errors.forEach((error) => {
            expect(error.row >= range.range.startRow).toBe(true);
            expect(error.row <= range.range.endRow).toBe(true);
            expect(error.column >= range.range.startColumn).toBe(true);
            expect(error.column <= range.range.endColumn).toBe(true);
            expect(error.sheetName).toBe(worksheet.getSheetName());
        });
    });
    ...
    it('should return empty array for range with no formula errors', () => {
        ...
        // Test range that contains no errors (only regular values)
        const range = {
            sheetId: worksheet.getSheetId(),
            range: {
                startRow: 0,
    ...
    it('should return empty array for invalid sheet range', () => {
        const workbook = univerAPI.getActiveWorkbook()!;
