...
export class Cumprinc extends BaseFunction {
    ...
    private _getResult(
        ...
        for (let i = _startPeriodValue; i <= endPeriodValue; i++) {
            result += typeValue === 1
                ? payment - (calculateFV(rateValue, i - 2, payment, pvValue, 1) - payment) * rateValue
                : payment - calculateFV(rateValue, i - 1, payment, pvValue, 0) * rateValue;
        }
