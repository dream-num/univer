...
describe('Test asinh function', () => {
    ...
    describe('Asinh', () => {
        it('Value is normal number', () => {
            ...
            expect(getObjectValue(result, true)).toBe(0.88137358702);
        });

        it('Value is number negative', () => {
            const value = NumberValueObject.create(-2);
            const result = testFunction.calculate(value);
            expect(getObjectValue(result, true)).toBe(-1.44363547518);
        });

        it('Value is number string', () => {
            const value = StringValueObject.create('0.5');
            const result = testFunction.calculate(value);
            expect(getObjectValue(result, true)).toBe(0.48121182506);
        });
        ...
        it('Value is array', () => {
            ...
            expect(getObjectValue(result, true)).toStrictEqual([
                [0.88137358702, ErrorType.VALUE, 1.03503789619, 0.88137358702, 0, 0],
                [0, 5.29834236561, 1.58611197042, ErrorType.VALUE, -1.81844645923, ErrorType.NAME],
            ]);
        });
    });
});
