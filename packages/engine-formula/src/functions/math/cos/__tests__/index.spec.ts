...
describe('Test cos function', () => {
    const textFunction = new Cos(FUNCTION_NAMES_MATH.COS);

    describe('Cos', () => {
        it('Value is normal', () => {
            const value = NumberValueObject.create(1);
            const result = textFunction.calculate(value);
            expect(result.getValue()).toBeCloseTo(Math.cos(1));
        });
        ...
        it('Value is array', () => {
            ...
            expect(transformToValue(result.getArrayValue())).toStrictEqual([
                [Math.cos(1), '#VALUE!', Math.cos(1.23), Math.cos(1), Math.cos(0)],
                [Math.cos(0), Math.cos(100), Math.cos(2.34), '#VALUE!', Math.cos(-3)],
            ]);
        });
    });
});
