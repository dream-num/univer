...
describe('Test csc function', () => {
    const textFunction = new Csc(FUNCTION_NAMES_MATH.CSC);

    describe('Csc', () => {
        it('Value is normal', () => {
            const value = NumberValueObject.create(1);
            const result = textFunction.calculate(value);
            expect(result.getValue()).toBeCloseTo(1.1883951057781212, 10);
        });
        ...
        it('Value is array', () => {
            ...
            expect(transformToValue(result.getArrayValue()).map((row) => row.map((value) => typeof value === 'number' ? Number(value.toFixed(15)) : value))).toStrictEqual([
                [1.1883951057781212, '#VALUE!', 1.5377805614452478, 1.1883951057781212, Infinity],
                [Infinity, 0.0100003333466672, 0.4270796165014858, '#VALUE!', -7.086167395737185],
