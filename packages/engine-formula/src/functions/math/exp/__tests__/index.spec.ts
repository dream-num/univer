...
describe('Test exp function', () => {
    ...
    describe('Exp', () => {
        it('Value is normal number', () => {
            ...
            expect(getObjectValue(result, true)).toBe(2.71828182846);
        });

        it('Value is number valid', () => {
            const value = NumberValueObject.create(-2);
            const result = testFunction.calculate(value);
            expect(getObjectValue(result, true)).toBe(0.135335283237);
        });

        it('Value is number string', () => {
            const value = StringValueObject.create('1.5');
            const result = testFunction.calculate(value);
            expect(getObjectValue(result, true)).toBe(4.48168907034);
        });
        ...
        it('Value is array', () => {
            ...
            expect(getObjectValue(result, true)).toStrictEqual([
                [2.71828182846, ErrorType.VALUE, 3.42122953629, 2.71828182846, 1, 1],
                [1, 2.6881171418161356e+43, 10.3812365627, ErrorType.VALUE, 0.0497870683679, ErrorType.NAME],
            ]);
        });
    });
});
