...
describe('Test varpa function', () => {
    const testFunction = new Varpa(FUNCTION_NAMES_STATISTICAL.VARPA);
    ...
    describe('Varpa', () => {
        it('Var1 is number, var2 is number', () => {
            ...
            expect(getObjectValue(result)).toBe(0.25);
        });
        ...
        it('Var1 is number, var2 is array not includes error', () => {
            ...
            const result = testFunction.calculate(var1, var2);
            expect(getObjectValue(result, true)).toBe(820.718914049587);
        });
    });
});
