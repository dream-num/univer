...
describe('context extra', () => {
    it('restores cached normalized fonts across nested drawing states without reading the native font again', () => {
        ...
        Object.defineProperty(nativeCtx, 'font', {
            get: readFont,
            set: (value: string) => {
                font = value.replace('normal ', '');
            },
        });
        vi.mocked(nativeCtx.save).mockImplementation(() => {
            stack.push(font);
        });
        vi.mocked(nativeCtx.restore).mockImplementation(() => {
            font = stack.pop() ?? font;
        });
        const context = new UniverRenderingContext2D(nativeCtx);
        context.font = 'normal 12px Arial';
        expect(context.font).toBe('12px Arial');
        context.save();
        context.font = 'normal 14px serif';
        expect(context.font).toBe('14px serif');
        context.save();
        context.scale(2, 2);
        context.restore();
        expect(context.font).toBe('14px serif');
        context.restore();
        expect(context.font).toBe('12px Arial');
        expect(readFont.mock.calls.length).toBe(2);

        context.font = 'normal 18px serif';
        context.save();
        context.font = '20px Arial';
        context.restore();
        expect(context.font).toBe('18px serif');
        // An unmatched native restore is a no-op, not a stale saved font.
        context.restore();
        expect(context.font).toBe('18px serif');
    });

    it('invalidates saved font values after a canvas reset or its size-reset transform', () => {
        const nativeCtx = createNativeContext();
        const context = new UniverRenderingContext2D(nativeCtx);
        context.font = '12px Arial';
        expect(context.font).toBe('12px Arial');
        context.save();
        // A canvas size assignment resets native state before Canvas.setSize sets its transform.
        nativeCtx.font = '10px sans-serif';
        ...
        context.save();
        vi.mocked(nativeCtx.reset).mockImplementation(() => {
            nativeCtx.font = '10px sans-serif';
        });
        context.reset();
        expect(context.font).toBe('10px sans-serif');
    ...
    it('falls back when the native context does not support roundRect', () => {
        ...
        renderCtx.roundRect(10, 20, 40, 20, 4);

        expect(nativeCtx.moveTo).toHaveBeenCalledWith(14, 20);
        expect(nativeCtx.quadraticCurveTo).toHaveBeenNthCalledWith(1, 50, 20, 50, 24);
        expect(nativeCtx.closePath).toHaveBeenCalledTimes(1);
        expect(nativeCtx.rect).not.toHaveBeenCalled();
    });
