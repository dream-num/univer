...
import { drawDiagonalLineByBorderType, drawLineByBorderType, getLineLengthByBorderType, getLineWidth, setLineType } from '../../../basics/draw';
...
export class Border extends SheetExtension {
    ...
    renderBorderByCell(renderBorderContext: IRenderBorderContext, row: number, col: number, borderCacheItem: BorderCache) {
        ...
        for (const key in borderCacheItem) {
            ...
            const lineLength = getLineLengthByBorderType(type, (lineWidth - 1) / 2 / precisionScale, {
                startX,
                startY,
                endX,
                endY,
            });
            setLineType(ctx, style, lineLength / precisionScale);
