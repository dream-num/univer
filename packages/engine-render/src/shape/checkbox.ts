...
export class Checkbox extends Shape<ICheckboxProps> {
    ...
    static override drawWith(ctx: UniverRenderingContext, props: ICheckboxProps) {
        ...
        if (checked) {
            const path = new Path2D(checkedPath);
            // Rect.drawWith(ctx, {
            //     ...extProps,
            //     radius: 2,
            //     width: width - 2,
            ...
            const path = new Path2D(uncheckedPath);
            ctx.stroke(path);
            // Rect.drawWith(ctx, {
            //     ...extProps,
            //     radius: 2,
