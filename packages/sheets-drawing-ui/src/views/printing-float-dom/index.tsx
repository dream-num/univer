...
export const FloatDomSingle = memo((props: { layer: IFloatDom; id: string; position: IFloatDomLayout }) => {
    ...
    return (
        <div style={{ position: 'absolute', top: 0, left: 0 }}>
            {floatDomParams.map(([id, floatDom]) => (
                <FloatDomSingle key={id} layer={floatDom} id={id} position={floatDom.position} />
            ))}
        </div>
    );
};
