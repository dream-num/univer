...
import { deriveSheetGroupRotateEnabledFromChildren } from '../rotate-enabled';
...
describe('sheet drawing rotate-enabled helpers', () => {
    it('treats old chart children as non-rotatable', () => {
        ...
        expect(deriveSheetGroupRotateEnabledFromChildren(root, [chart])).toBe(false);
    });
    ...
    it('recomputes stale nested group rotate flags from current children', () => {
        ...
        expect(deriveSheetGroupRotateEnabledFromChildren(root, [nested, child])).toBe(true);
    });
});
