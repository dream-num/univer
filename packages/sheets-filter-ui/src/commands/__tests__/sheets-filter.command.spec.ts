...
import { NORMAL_SELECTION_PLUGIN_NAME, SelectionManagerService, SetRangeValuesCommand, SetRangeValuesMutation, SheetInterceptorService, SheetPermissionService } from '@univerjs/sheets';
...
import { ClearSheetsFilterCriteriaCommand, ReCalcSheetsFilterConditionsCommand, SetSheetsFilterCriteriaCommand, SmartToggleSheetsFilterCommand } from '../sheets-filter.command';
...
describe('test sheets filter commands', () => {
    ...
    describe('test "ReCalcSheetsFilterConditionsCommand"', () => {
        it('should reCalc after set range values and get correct result', async () => {
            ...
            expect(await commandService.executeCommand(ReCalcSheetsFilterConditionsCommand.id, {
                unitId: 'test',
                subUnitId: 'sheet1',
            })).toBeTruthy();
            expect(getFilterModel()!.filteredOutRows).toEqual(new Set([2, 3, 4, 5]));
