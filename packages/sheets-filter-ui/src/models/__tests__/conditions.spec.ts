...
import { BooleanNumber, CustomFilterOperator } from '@univerjs/core';
    ...
    describe('test utils work as expected in different condition format situations', () => {
        ...
        describe('test NOT_EMPTY condition', () => {
            it('should load NOT_EMPTY condition from custom filter with space string', () => {
                expect(FilterConditionItems.testMappingFilterColumn({ customFilters: { customFilters: [{ operator: CustomFilterOperator.NOT_EQUAL, val: ' ' }] } }))
                ...
                expect(FilterConditionItems.testMappingFilterColumn({
                    customFilters: { customFilters: [{ operator: CustomFilterOperator.NOT_EQUAL, val: '123' }] },
                })).toEqual([FilterConditionItems.NOT_EQUAL, { operator1: CustomFilterOperator.NOT_EQUAL, val1: '123' }]);
            });
            ...
            it('should map to filter column with space string', () => {
                expect(FilterConditionItems.NOT_EMPTY.mapToFilterColumn({ operator1: ExtendCustomFilterOperator.NOT_EMPTY }))
                    .toEqual({ customFilters: { customFilters: [{ operator: CustomFilterOperator.NOT_EQUAL, val: ' ' }] } });
            });
        });
        ...
        describe('test DOES_NOT_CONTAIN condition', () => {
            it('should load DOES_NOT_CONTAIN condition from char set "*xxx*"', () => {
                expect(FilterConditionItems.testMappingFilterColumn({ customFilters: { customFilters: [{ val: '*xxx*', operator: CustomFilterOperator.NOT_EQUAL }] } }))
            ...
            it('should map to filter column with char set "*xxx*"', () => {
                expect(FilterConditionItems.DOES_NOT_CONTAIN.mapToFilterColumn({ operator1: ExtendCustomFilterOperator.DOES_NOT_CONTAIN, val1: 'univer' }))
                    .toEqual({ customFilters: { customFilters: [{ val: '*univer*', operator: CustomFilterOperator.NOT_EQUAL }] } });
                ...
                expect(FilterConditionItems.DOES_NOT_CONTAIN.mapToFilterColumn({ operator1: ExtendCustomFilterOperator.DOES_NOT_CONTAIN, val1: ' ' }))
                    .toEqual({ customFilters: { customFilters: [{ val: '* *', operator: CustomFilterOperator.NOT_EQUAL }] } });
            });
        });
        ...
        describe('test EQUAL condition', () => {
            ...
            it('should mapped params with only one operator EQUAL', () => {
                ...
                expect(FilterConditionItems.EQUAL.testMappingParams({ operator1: CustomFilterOperator.NOT_EQUAL })).toBeFalsy();
        ...
        describe('test NOT_EQUAL', () => {
            it('should load NOT_EQUAL condition from operator"', () => {
                expect(FilterConditionItems.testMappingFilterColumn({ customFilters: { customFilters: [{ val: '123', operator: CustomFilterOperator.NOT_EQUAL }] } }))
                    .toEqual([FilterConditionItems.NOT_EQUAL, { operator1: CustomFilterOperator.NOT_EQUAL, val1: '123' }]);
            });

            it('should support default params with empty string', () => {
                expect(FilterConditionItems.NOT_EQUAL.getDefaultFormParams()).toEqual({ operator1: CustomFilterOperator.NOT_EQUAL, val1: '' });
            });

            it('should mapped params with only one operator NOT_EQUAL', () => {
                expect(FilterConditionItems.NOT_EQUAL.testMappingParams({ operator1: CustomFilterOperator.NOT_EQUAL })).toBeTruthy();
                expect(FilterConditionItems.NOT_EQUAL.testMappingParams({ operator1: CustomFilterOperator.EQUAL })).toBeFalsy();
                expect(() => FilterConditionItems.NOT_EQUAL.testMappingParams({ operator1: CustomFilterOperator.NOT_EQUAL, operator2: CustomFilterOperator.NOT_EQUAL }))
                    .toThrowError();

                expect(FilterConditionItems.NOT_EQUAL.testMappingParams({ operator2: CustomFilterOperator.NOT_EQUAL })).toBeTruthy();
            });

            it('should map to filter column with operator NOT_EQUAL', () => {
                expect(FilterConditionItems.NOT_EQUAL.mapToFilterColumn({ operator1: CustomFilterOperator.NOT_EQUAL, val1: '123' }))
                    .toEqual({ customFilters: { customFilters: [{ val: '123', operator: CustomFilterOperator.NOT_EQUAL }] } });
            });
        });
