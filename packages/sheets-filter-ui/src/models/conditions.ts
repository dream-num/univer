...
import type { ICustomFilters, IFilterColumn, Nullable } from '@univerjs/core';
...
export interface IFilterConditionItem {
     ...
     * @param mapParams
     */
    mapToFilterColumn(mapParams: IFilterConditionFormParams): Omit<IFilterColumn, 'colId'>;
    /**
...
export namespace FilterConditionItems {
    ...
    export const NOT_EMPTY: IFilterConditionItem = {
        ...
        mapToFilterColumn: () => ({ customFilters: { customFilters: [{ val: ' ', operator: CustomFilterOperator.NOT_EQUAL }] } }),
        testMappingFilterColumn: (filterColumn) => {
            if (filterColumn.customFilters?.customFilters.length !== 1) {
                return false;
            }
            ...
            const firstCustomFilter = filterColumn.customFilters.customFilters[0];
            const canMap = firstCustomFilter.val === ' ' && firstCustomFilter.operator === CustomFilterOperator.NOT_EQUAL;
            if (!canMap) {
                return false;
            }
    ...
    export const DOES_NOT_CONTAIN: IFilterConditionItem = {
        ...
        mapToFilterColumn: (mapParams) => ({
            customFilters: { customFilters: [{ val: `*${mapParams.val1}*`, operator: CustomFilterOperator.NOT_EQUAL }] },
        }),
        testMappingParams: (params) => {
            const [op] = getOnlyOperatorAndVal(params);
        ...
        testMappingFilterColumn: (filterColumn) => {
            ...
            const valAsString = firstCustomFilter.val.toString();
            if (
                firstCustomFilter.operator === CustomFilterOperator.NOT_EQUAL
    ...
    export const NOT_EQUAL: IFilterConditionItem = {
        ...
        getDefaultFormParams: () => ({ operator1: CustomFilterOperator.NOT_EQUAL, val1: '' }),
        testMappingParams: (params) => {
            const [op] = getOnlyOperatorAndVal(params);
            return op === CustomFilterOperator.NOT_EQUAL;
        },
        ...
        testMappingFilterColumn: (filterColumn) => {
            ...
            const firstCustomFilter = filterColumn.customFilters.customFilters[0];
            if (firstCustomFilter.operator !== CustomFilterOperator.NOT_EQUAL) {
                return false;
            }
    ...
    export function mapToFilterColumn(condition: IFilterConditionItem, mapParams: IFilterConditionFormParams): Omit<IFilterColumn, 'colId'> {
        // TODO@wzhudev: basic params validation
    ...
    export function testMappingFilterColumn(filterColumn: Nullable<Omit<IFilterColumn, 'colId'>>): [IFilterConditionItem, IFilterConditionFormParams] {
        ...
        throw new Error('[SheetsFilter]: no condition item can be mapped from the filter column!');
    }
