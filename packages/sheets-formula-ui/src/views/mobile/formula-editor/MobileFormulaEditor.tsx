...
import type { FormulaSelectingType } from '../../formula-editor/hooks/use-formula-selection';
import type { IRefSelection } from '../../formula-editor/hooks/use-highlight';
import {
    BuildTextUtils,
...
import { EMBEDDING_FORMULA_EDITOR, MOBILE_FORMULA_FUNCTION_PANEL_OPEN } from '@univerjs/sheets-ui';
...
} from '../../formula-editor/formula-embed-integration.service';
import { hasActiveFormulaEmbedInteraction, shouldRefocusFormulaEditorOnMouseUp, useFocus } from '../../formula-editor/hooks/use-focus';
import { getSelectionAfterLaggingFormulaInput, useFormulaSelecting } from '../../formula-editor/hooks/use-formula-selection';
import { useFormulaToken } from '../../formula-editor/hooks/use-formula-token';
import { useDocHight, useSheetHighlight } from '../../formula-editor/hooks/use-highlight';
import { useLeftAndRightArrow } from '../../formula-editor/hooks/use-left-and-right-arrow';
import { useRefactorEffect } from '../../formula-editor/hooks/use-refactor-effect';
import { useResetSelection } from '../../formula-editor/hooks/use-reset-selection';
import { useSheetSelectionChange } from '../../formula-editor/hooks/use-sheet-selection-change';
import { useStateRef } from '../../formula-editor/hooks/use-state-ref';
import { useSwitchSheet } from '../../formula-editor/hooks/use-switch-sheet';
import { useVerify } from '../../formula-editor/hooks/use-verify';
import { getFormulaText } from '../../formula-editor/utils/get-formula-text';
...
import { MobileSearchFunction } from './search-function/MobileSearchFunction';

export interface IMobileFormulaEditorProps {
    unitId: string;
    subUnitId: string;
    initValue: `=${string}`;
    autofocus?: boolean;
...
export interface IMobileFormulaEditorRef {
...
export function restoreMobileFormulaEditorHostFocus(
    editorService: Pick<IEditorService, 'blur' | 'getFocusId'>,
    univerInstanceService: Pick<IUniverInstanceService, 'focusUnit' | 'getCurrentUnitOfType' | 'getUnit'>,
    editorId: string,
    hostUnitId: string
): void {
    ...
    if (
        currentWorkbook?.getUnitId() !== hostUnitId ||
        !univerInstanceService.getUnit(hostUnitId, UniverInstanceType.UNIVER_SHEET)
    ) {
        return;
    }

    univerInstanceService.focusUnit(hostUnitId);
}
...
export const MobileFormulaEditor = forwardRef((props: IMobileFormulaEditorProps, ref: Ref<IMobileFormulaEditorRef>) => {
    const {
        errorText,
        initValue,
        unitId,
    ...
    }, [formulaText, onMobileFormulaActiveChange]);
    ...
    useLayoutEffect(() => {
        ...
        return () => {
            ...
            restoreMobileFormulaEditorHostFocus(editorService, univerInstanceService, editorId, unitId);
            dispose?.dispose();
        };
    }, []);
    ...
    }, [applyMobileInsertion, editor, mobileOperatorRequest]);
