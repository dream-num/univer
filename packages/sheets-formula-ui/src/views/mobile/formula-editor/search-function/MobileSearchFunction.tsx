...
import { useEditorPosition } from '../../../formula-editor/hooks/use-editor-position';
import { useFormulaSearch } from '../../../formula-editor/hooks/use-formula-search';
import { useStateRef } from '../../../formula-editor/hooks/use-state-ref';

interface IMobileSearchFunctionProps {
    isFocus: boolean;
...
export const MobileSearchFunction = forwardRef<HTMLElement, IMobileSearchFunctionProps>(MobileSearchFunctionFactory);
function MobileSearchFunctionFactory(props: IMobileSearchFunctionProps, ref: any) {
    ...
    return searchList.length > 0 && visible && (
        <RectPopup portal anchorRect$={position$} direction="vertical">
            <ul
                ...
                `, 'univer-max-h-[38vh] univer-w-[calc(100vw-16px)]', borderClassName, scrollbarClassName)}
            >
                {searchList.map((item, index) => (
                    <li
                        ...
                        `, 'univer-flex univer-min-h-12 univer-flex-col univer-justify-center univer-px-3 univer-py-2', {
