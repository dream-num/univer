...
export default {
    ...
    MATCH: {
        description: 'MATCH 関数は、範囲 のセルの範囲で指定した項目を検索し、その範囲内の項目の相対的な位置を返します。 たとえば、範囲 A1:A3 に値 5、25、38 が含まれている場合、数式「=MATCH(25,A1:A3,0)」を入力すると、範囲内では 25 が 2 番目の項目であるため、数字 2 が返されます。',
        ...
        functionParameter: {
            lookupValue: { name: '検査値', detail: '検査範囲の中で照合する値を指定します。 たとえば、電話番号帳を使ってある人の電話番号を調べるとき、検査値としてその人の氏名を指定しますが、実際に検索するのは電話番号です。検査値には、値 (数値、文字列、または論理値)、またはこれらの値に対するセル参照を指定できます。' },
            lookupArray: { name: '検査範囲', detail: '検索するセルの範囲を指定します。' },
            matchType: { name: '照合の型', detail: '-1、0、1 の数値のいずれかを指定します。 照合の型には、検査範囲の中で検査値を探す方法を指定します。 この引数の既定値は 1 です。' },
        },
    },
    OFFSET: {
        ...
        functionParameter: {
            reference: { name: '参照', detail: 'オフセットの基準となる参照を指定します。 参照は、セルまたは隣接するセル範囲を参照する必要があり、それ以外の場合は、エラー値 #VALUE! が返されます。' },
            rows: { name: '行数', detail: '基準の左上隅のセルを上方向または下方向へシフトする距離を行数単位で指定します。 行数に 5 を指定すると、オフセット参照の左上隅のセルは、基準の左上隅のセルから 5 行下方向へシフトします。 行数に正の数を指定すると開始位置の下方向へシフトし、負の数を指定すると開始位置の上方向へシフトします。' },
            cols: { name: '列数', detail: '結果の左上隅のセルを左方向または右方向へシフトする距離を列数単位で指定します。 列数に 5 を指定すると、オフセット参照の左上隅のセルは、基準の左上隅のセルから 5 列右方向へシフトします。 列数に正の数を指定すると開始位置から右方向へシフトし、負の数を指定すると開始位置から左方向へシフトします。' },
