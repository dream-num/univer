...
import { BooleanNumber, type IWorkbookData, LocaleType, type Nullable, Univer, UniverInstanceType, type Workbook } from '@univerjs/core';
import {
    type IViewportInfo,
    RENDER_CLASS_TYPE,
...
import { SpreadsheetRenderSkeleton } from '../sheet.render-skeleton';
...
describe('SheetComponent', () => {
    ...
    beforeEach(() => {
        ...
        if (!worksheet) {
            throw new Error('Active sheet is required for sheet component tests');
        }
    ...
    it('updates the skeleton and syncs the scene transformer offset', () => {
        ...
        expect(scene.updateTransformerZero).toHaveBeenCalledWith(skeleton.rowHeaderWidth, skeleton.columnHeaderHeight);
    });

    it('uses viewer ancestor scale for hit testing helpers and clears skeleton on dispose', () => {
        const component = new TestSheetComponent('sheet-component', skeleton);
        component.parent = {
