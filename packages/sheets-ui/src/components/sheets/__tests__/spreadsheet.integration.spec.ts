...
import { createSheetGapTestConfig } from '../sheet-render-skeleton.base';
import {
...
function createFixture(): IFixture {
    ...
    const modelSkeleton = new SpreadsheetSkeleton(worksheet).calculate();
    const skeleton = injector.createInstance(SpreadsheetRenderSkeleton, modelSkeleton, workbook.getStyles()).calculate();
...
describe('spreadsheet integration', () => {
    ...
    it('builds border cache for row-wise merged cells on the selection left edge', () => {
        ...
        const worksheet = workbook.getActiveSheet()!;
        const modelSkeleton = new SpreadsheetSkeleton(worksheet).calculate();
        const skeleton = fixture.univer.__getInjector().createInstance(SpreadsheetRenderSkeleton, modelSkeleton, workbook.getStyles()).calculate();
    ...
    it('does not recalculate model geometry while scrolling', () => {
        const { spreadsheet, skeleton, mainCanvas, cacheCanvas, scene } = fixture;
        const context = mainCanvas.getContext();
        const calculateSpy = vi.spyOn(skeleton.spreadsheetSkeleton, 'calculate');
        const viewportInfo = createViewportInfo(scene, cacheCanvas, {
            diffBounds: [createBound(100, 60, 220, 140)],
            diffCacheBounds: [createBound(100, 60, 220, 140)],
            diffX: 0,
