...
export class SheetRenderSkeletonBase extends Skeleton {
    ...
    override makeDirty(state: boolean): void {
        super.makeDirty(state);
        if (state) {
            this.spreadsheetSkeleton.makeDirty(true);
        }
    }
    ...
    private _addVisualGaps(
        accumulation: number[],
        gaps: IObjectArrayPrimitiveType<IGapItem> | undefined
    ): number[] {
        ...
        let gapTotal = 0;
        return accumulation.map((offset, index) => {
            gapTotal += gaps[index]?.size ?? 0;
            return offset + gapTotal;
        });
    }
    ...
    protected _updateLayout(): void {
        ...
        const { rowHeader, columnHeader } = this._worksheetData;
        this.spreadsheetSkeleton.calculate();
        const rowHeightAccumulation = this._addVisualGaps(
            this.spreadsheetSkeleton.rowHeightAccumulation,
            this._gapConfig.rowGaps
        );
        const columnWidthAccumulation = this._addVisualGaps(
            this.spreadsheetSkeleton.columnWidthAccumulation,
            this._gapConfig.colGaps
        );
        ...
        this._rowTotalHeight = rowHeightAccumulation.at(-1) ?? 0;
        this._rowHeightAccumulation = rowHeightAccumulation;
        this._columnTotalWidth = columnWidthAccumulation.at(-1) ?? 0;
