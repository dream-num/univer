    ...
    return (
        ...
        >
            <header
                ...
                `}
            >
                <div>{localeService.t<LocaleKey>('slides-ui.image-panel.transform.title')}</div>
            </header>
            ...
            >
                <div>
                    <span>{localeService.t<LocaleKey>('slides-ui.image-panel.transform.width')}</span>
                    <InputNumber
                        min={1}
                        value={width}
                ...
                <div>
                    <span>{localeService.t<LocaleKey>('slides-ui.image-panel.transform.height')}</span>
                    <InputNumber
                        min={1}
                        value={height}
            ...
            <div className="univer-relative univer-mt-2.5 univer-flex univer-h-full">
                <div>
                    <span>{localeService.t<LocaleKey>('slides-ui.image-panel.transform.x')}</span>
                    <InputNumber min={0} precision={1} value={xPosition} onChange={(val) => { handleXChange(val); }} />
                </div>
                <div>
                    <span>{localeService.t<LocaleKey>('slides-ui.image-panel.transform.y')}</span>
                    <InputNumber min={0} precision={1} value={yPosition} onChange={(val) => { handleYChange(val); }} />
                </div>
                <div>
                    <span>{localeService.t<LocaleKey>('slides-ui.image-panel.transform.rotate')}</span>
                    <InputNumber
                        precision={1}
                        value={rotation}
