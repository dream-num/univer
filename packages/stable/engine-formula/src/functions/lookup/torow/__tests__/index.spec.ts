...
describe('Test torow function', () => {
    const testFunction = new Torow(FUNCTION_NAMES_LOOKUP.TOROW);
    ...
    describe('Torow', () => {
        it('Value is normal', async () => {
            ...
            expect(getObjectValue(result)).toStrictEqual([
                ['Ben', 1, 'Mary', -2, true, 'James', false, 'Harry', 1.23, '彭德威'],
            ]);
        });
        ...
        it('Ignore value test', async () => {
            ...
            const result = testFunction.calculate(array, ignore, scanByColumn);
            expect(getObjectValue(result)).toStrictEqual([
                ['Ben', 1, 'Mary', -2, true, ErrorType.NAME, 0, 'James', false, 'Harry', 1.23, '彭德威'],
            ]);

            const ignore2 = NumberValueObject.create(1);
            const result2 = testFunction.calculate(array, ignore2, scanByColumn);
            expect(getObjectValue(result2)).toStrictEqual([
                ['Ben', 1, 'Mary', -2, true, ErrorType.NAME, 'James', false, 'Harry', 1.23, '彭德威'],
            ]);
            ...
            expect(getObjectValue(result3)).toStrictEqual([
                ['Ben', 1, 'Mary', -2, true, 0, 'James', false, 'Harry', 1.23, '彭德威'],
            ]);
        ...
        it('ScanByColumn value test', async () => {
            ...
            const result = testFunction.calculate(array, ignore, scanByColumn);
            expect(getObjectValue(result)).toStrictEqual([
                ['Ben', true, false, 1, 'Harry', 'Mary', 1.23, -2, 'James', '彭德威'],
            ]);
