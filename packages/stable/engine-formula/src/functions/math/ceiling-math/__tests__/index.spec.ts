...
import { CeilingMath } from '../index';

describe('Test ceiling.math function', () => {
    const testFunction = new CeilingMath(FUNCTION_NAMES_MATH.CEILING_MATH);

    describe('CeilingMath', () => {
        ...
        it('number and significance is array, mode is not null or 0', () => {
            ...
            const result = testFunction.calculate(number, significance, mode);
            expect(transformToValue(result.getArrayValue())).toStrictEqual([
                [3, 0, -2, 0, ErrorType.VALUE, 2.5, -51, 6, ErrorType.NA],
            ]);
        });
    });
});
