...
describe('Test fixed function', () => {
    const testFunction = new Fixed(FUNCTION_NAMES_TEXT.FIXED);

    describe('Fixed', () => {
        it('Value is normal', () => {
            const number = NumberValueObject.create(1234.567);
            const decimals = NullValueObject.create();
            const noCommas = NullValueObject.create();
            const result = testFunction.calculate(number, decimals, noCommas);
        ...
        it('Decimals value test', () => {
            ...
            const result2 = testFunction.calculate(number, decimals2, noCommas);
            expect(getObjectValue(result2)).toStrictEqual('0');

            const decimals3 = NumberValueObject.create(4);
            const result3 = testFunction.calculate(number, decimals3, noCommas);
        ...
        it('NoCommas value test', () => {
            const number = NumberValueObject.create(1234.567);
            const decimals = NullValueObject.create();
            const noCommas = BooleanValueObject.create(true);
            const result = testFunction.calculate(number, decimals, noCommas);
            expect(getObjectValue(result)).toStrictEqual('1234.57');
        });
        ...
        it('Value is error', () => {
            ...
            const result2 = testFunction.calculate(number2, decimals2, noCommas);
            expect(getObjectValue(result2)).toStrictEqual(ErrorType.NAME);

            const noCommas2 = ErrorValueObject.create(ErrorType.NAME);
            const result3 = testFunction.calculate(number, decimals2, noCommas2);
