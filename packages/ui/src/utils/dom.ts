...
export function clientRect$(dom: HTMLElement): Observable<void> {
    const rect$: Nullable<Observable<void>> = (new Observable((subscriber) => {
        if (!dom) {
            throw new Error('[dom]: cannot subscribe to rect changes when dom is not set!');
        }

        const sub = observeClientRect(dom).subscribe(() => subscriber.next());

        return () => {
            sub.unsubscribe();
        };
    }));
    return rect$.pipe(shareReplay(1));
}
